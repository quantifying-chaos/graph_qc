...
from sympy import symbols, Poly, simplify, collect, Eq, O, RR
...
def t_psi(x):
    return -alpha_val * psi(psi(-x / alpha_val))
...
y_vals = [psi(x) for x in x_vals]
y_2_vals = [t_psi(x) for x in x_vals]
fig, ax = plt.subplots()
ax.plot(x_vals, y_vals, "--", label=r'$\psi$', alpha=0.5)
ax.plot(x_vals, y_2_vals, "-", alpha=0.5, label=r'$T\psi$')
