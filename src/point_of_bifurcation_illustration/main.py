...
def graph_bifurcation_comparison_plot(iter_times, lambda_1, lambda_2, x_low, x_high, fig_name):
    ...
    #####
    ax[0].plot(I, [iterate(logistic, lambda_1, x, iter_times)
               for x in I], label='$L_{\lambda}^%d$' % iter_times)
    ax[0].plot(I, [iterate(logistic, lambda_1, x, 2*iter_times)
               for x in I], label='$L_{\lambda}^%d$' % (2 * iter_times))
    ...
    ax[1].plot(I, [iterate(logistic, lambda_2, x, iter_times)
               for x in I], label='$L_{\lambda}^%d$' % iter_times)
    ax[1].plot(I, [iterate(logistic, lambda_2, x, 2*iter_times)
               for x in I], label='$L_{\lambda}^%d$' % (2 * iter_times))
    # remove x ticks
...
graph_bifurcation_comparison_plot(
    1, lambda_1, lambda_2, x_low, x_high, fig_name)
